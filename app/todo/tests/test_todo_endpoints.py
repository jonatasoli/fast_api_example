...
def test_error_route(client):
    response = client.get('/todo/error_route', headers=HEADERS)
...
def add_task(client):
    ...
    response = client.post('/todo/add', headers=HEADERS, json=data.dict())
    return response
...
def test_add_task_error_validate(client):
    ...
    response = client.post('/todo/add', headers=HEADERS, json=task)
...
def test_update_task(client):
    ...
    response = client.put(
        f"/todo/update/{response.json()['id']}",
        headers=HEADERS,
        json=data.dict(),
    )
    assert response.status_code == status.HTTP_200_OK
